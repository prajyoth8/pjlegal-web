...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Combine results
    // Update the combine results section in analytics/route.ts
const analyticsData = sessionData.map((session: { date: string; unique_count: number; total_count: number; new_count: number }) => {
  const booking = bookingData.find((b: { date: string; count: number }) => b.date === session.date) || { count: 0 };
  return {
    date: session.date,
