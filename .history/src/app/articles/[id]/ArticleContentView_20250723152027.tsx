// app/articles/[id]/ArticleContentView.tsx
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import rehypeRaw from 'rehype-raw';
import Image from 'next/image';

export default function ArticleContentView({ article }: { article: any }) {
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="text-neutral-400 text-sm mb-4 flex justify-between">
        <span>
          By <span className="text-cyan-400 font-medium hover:underline">Adv. R. Prajyoth Kumar</span>
        </span>
        <span>
      ...
      <div className="flex gap-4 mt-10 border-t border-neutral-800 pt-6">
        <a
          href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.title)}&url=${encodeURIComponent(`https://pjlegal.in/articles/${article.id}`)}`}
          target="_blank"
        ...
        <a
          href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://pjlegal.in/articles/${article.id}`)}`}
          target="_blank"
          rel="noopener noreferrer"
