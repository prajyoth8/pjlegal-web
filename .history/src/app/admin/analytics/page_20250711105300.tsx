    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 to-gray-900 text-gray-100 p-4 md:p-8 space-y-6">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Real-time analytics and model insights
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="border-gray-800 bg-gray-900/50 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              <span>Response Volume (Daily)</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="h-80">
            {loading ? (
        ...
        <Card className="border-gray-800 bg-gray-900/50 backdrop-blur-sm lg:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Icons.lineChart className="w-5 h-5 text-amber-400" />
              <span>Average Latency Over Time (ms)</span>
            </CardTitle>
          </CardHeader>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={<Icons.clock className="w-6 h-6 text-amber-400" />}
          loading={loading}
        />
        <StatCard
          title="Top Model"
          value={modelBreakdown[0]?.name || "N/A"}
          icon={<Icons.cpu className="w-6 h-6 text-blue-400" />}
          loading={loading}
        />
        <StatCard
          title="Cache Hit Rate"
          value={
            responseSource.length === 2
              ? `${Math.round(
                  (responseSource[0].value / (responseSource[0].value + responseSource[1].value)) * 100
                }%`
              : "N/A"
          }
