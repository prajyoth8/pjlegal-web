...
export default function UsersPage() {
  ...
  const fetchData = async () => {
    const { data: chatbot_sessions } = await supabase.from("chatbot_sessions").select("*");
  ...
  const filteredUserList = useMemo(() => {
    return Object.values(groupedUsers)
      ...
      .filter(
        (user: any) => !searchTerm || user.email.toLowerCase().includes(searchTerm.toLowerCase())
      );
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(value, name, props) => [value, `${name} Consultations`]}
                    contentStyle={{
                      borderRadius: "12px",
                      boxShadow: "0 4px 20px rgba(0,0,0,0.1)",
                    }}
        ...
        <div className="space-y-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredUserList.slice(0, 5).map((user: any, i) => (
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <table className="w-full">
             <thead className="bg-gray-50">
               <tr>
                 <th px-6 py-4 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider>#</th>
                 <th px-6 py-4 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider">Name</th>
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border-t-4 border-red-500">
            ...
            {/* ... similar enhanced table structure for offensive queries ... */}
          </div>
        </div>

        {/* Floating Action Button */}
        <div className="fixed bottom-8 right-8">
          <button className="p-4 bg-blue-600 text-white rounded-full shadow-xl hover:bg-blue-700 transition-colors flex items-center justify-center">
