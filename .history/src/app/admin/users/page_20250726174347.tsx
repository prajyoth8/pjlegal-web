...
export default function UsersPage() {
  ...
  // ✅ Group requests by email with count
  const usageByEmail = filteredRequests.reduce((acc: Record<string, number>, item) => {
    const email = item.user_email || "Unknown";
    acc[email] = (acc[email] || 0) + 1;
    return acc;
  }, {});

  const usageArray = Object.entries(usageByEmail)
    .filter(([email]) => email.toLowerCase().includes(searchTerm.toLowerCase()))
    .map(([email, count]) => ({ email, count }));

  const chartData = Object.entries(
    filteredRequests.reduce((acc: any, item) => {
  ...
  return (
    <div className="p-6 max-w-screen-xl mx-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold mb-2">User Chatbot Usage</h1>
        <Link
      ...
      <div className="mb-6">
        <h2 className="font-semibold mb-2">📊 Chatbot Usage Over Time</h2>
      ...
      <div>
        <h2 className="text-xl font-semibold mb-2">🤖 Chatbot Users Summary</h2>
        <table className="w-full text-sm border">
          <thead className="bg-gray-100 font-semibold">
            <tr>
              <th className="p-2">#</th>
              <th className="p-2">Email</th>
              <th className="p-2">Usage Count</th>
            </tr>
          </thead>
          <tbody>
            {usageArray.map((item, i) => (
