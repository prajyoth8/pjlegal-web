...
export default function UserAnalyticsPage() {
  ...
  const fetchData = async () => {
    ...
    const { data: chatbotSessionsRaw } = await supabase.from("chatbot_sessions").select("*");
const { data: chatbotRequestsRaw } = await supabase.from("chatbot_requests").select("id, session_id, prompt");
const { data: consultationBookingsRaw } = await supabase.from("consultation_bookings").select("*");

const chatbotSessions = chatbotSessionsRaw ?? [];
const chatbotRequests = chatbotRequestsRaw ?? [];
const consultationBookings = consultationBookingsRaw ?? [];
