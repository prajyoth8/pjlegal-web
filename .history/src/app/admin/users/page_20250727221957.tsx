...
export default function UsersPage() {
  ...
  const filterData = (data: any[], fieldsToSearch: string[]) => {
    ...
    return periodFiltered.filter((item) =>
      fieldsToSearch.some((field) => {
        ...
        if (field.includes(".")) {
          // Handle nested fields (e.g., 'users.email')
          const [parent, child] = field.split(".");
          return String(item[parent]?.[child] || "")
            .toLowerCase()
  ...
  const filteredRequests = useMemo(
    () => filterData(chatbotRequests, ["query"]),
    [chatbotRequests, period, searchTerm]
  );
                  ...
                  {filteredConsultations.length > 0 ? (
                    filteredConsultations.map((item, i) => (
                      <tr key={i} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="p-3">{highlightMatch(item.email || "-", searchTerm)}</td>
              ...
              <h2 className="text-xl font-semibold flex items-center gap-2 text-red-600">
                ...
                {searchTerm && (
                  <span className="text-sm text-red-500 ml-2">
                    ({filteredOffensives.length} results)
                  </span>
                )}
              </h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-red-100">
                  {filteredOffensives.length > 0 ? (
                    filteredOffensives.map((item, i) => (
                    ...
                    <tr>
                      <td colSpan={4} className="p-6 text-center text-gray-500">
                        <div className="flex flex-col items-center justify-center py-8">
                          ...
                          <p className="text-gray-600">
                            {searchTerm
                              ? "No offensive queries match your search"
                              : "No offensive queries found"}
                          </p>
                          {searchTerm && (
                            <button
                              onClick={clearSearch}
