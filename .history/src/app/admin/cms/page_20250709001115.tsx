...
export default function CMSPage() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-purple flex items-center justify-center">
        <div className="flex flex-col items-center">
          <div className="w-12 h-12 border-4 border-amber-500 border-t-transparent rounded-full animate-spin mb-4"></div>
          <p className="text-amber-800 font-medium">Loading CMS Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <AdminAuthGuard>
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-purple">
        {/* Header */}
        <header className="bg-white/80 backdrop-blur-sm shadow-sm">
          <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex justify-between items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-amber-900">Legal Content Management</h1>
              <p className="text-sm text-amber-700">Welcome back, {userEmail}</p>
            </div>
            <button
              onClick={async () => {
                await supabase.auth.signOut();
              ...
              className="px-4 py-2 text-sm font-medium text-amber-700 hover:text-amber-900 hover:bg-amber-100 rounded-md transition-colors"
            >
              Sign out
            </button>
          </div>
        ...
        <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          ...
          <section className="mb-8">
            <div className="bg-white/80 backdrop-blur-sm rounded-lg p-6 shadow">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                <div>
                  <h2 className="text-lg font-medium text-amber-900">Content Scraper</h2>
                ...
                <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                  <select
                    value={selectedSource}
                    onChange={(e) => setSelectedSource(e.target.value)}
                    className="block w-full rounded-md border-amber-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-sm py-2 px-3 border bg-white"
                    disabled={isScraping}
                  >
                    <option value="">Select a source...</option>
                    {sources.map((s) => (
                  ...
                  <button
                    onClick={triggerScraper}
                    disabled={!selectedSource || isScraping}
                    className="inline-flex items-center justify-center rounded-md border border-transparent bg-amber-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed min-w-[120px] transition-colors"
                  >
                    {isScraping ? (
                      <>
                        <svg
          ...
          {activeTab === "logs" ? (
            <section className="mb-8 bg-white/80 backdrop-blur-sm shadow rounded-lg overflow-hidden">
              <div className="px-6 py-4 border-b border-amber-200 flex justify-between items-center">
                <h3 className="text-lg font-medium text-amber-900">Scraper Activity</h3>
                <span className="text-sm text-amber-700">{logs.length} entries</span>
              </div>
              <div className="overflow-x-auto">
                  ...
                  <tbody className="bg-white divide-y divide-amber-200">
                    {logs.map((log) => (
                      <tr key={log.id} className="hover:bg-amber-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm text-amber-800 max-w-xs truncate">
                          {log.details}
                        </td>
                      </tr>
                    ))}
            ...
            <section className="mb-8 bg-white/80 backdrop-blur-sm shadow rounded-lg overflow-hidden">
              <div className="px-6 py-4 border-b border-amber-200 flex justify-between items-center">
                <h3 className="text-lg font-medium text-amber-900">Legal Documents</h3>
                <span className="text-sm text-amber-700">{laws.length} entries</span>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-amber-200">
                  <thead className="bg-amber-50">
                  ...
                  <tbody className="bg-white divide-y divide-amber-200">
                    {laws.map((law) => (
                      <tr key={law.id} className="hover:bg-amber-50/50 transition-colors">
                        <td className="px-6 py-4 text-sm font-medium text-amber-900 max-w-xs">
                          {law.title}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-amber-800">
                          {law.published_date
          ...
          <section className="bg-white/80 backdrop-blur-sm shadow rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-amber-200">
              <h3 className="text-lg font-medium text-amber-900">Create New Content</h3>
            </div>
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Manual Legal Entry */}
                <div className="border border-amber-200 rounded-lg p-6 bg-white/50">
                  <div className="flex items-center mb-4">
                    <div className="flex-shrink-0 bg-amber-600 rounded-md p-2">
                      <svg
                        className="h-5 w-5 text-white"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                    ...
                    <h4 className="ml-3 text-base font-medium text-amber-900">Legal Document</h4>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <label
                      ...
                      <input
                        ...
                        value={pasteTitle}
                        onChange={(e) => setPasteTitle(e.target.value)}
                        className="block w-full rounded-md border-amber-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-sm py-2 px-3 border bg-white/70"
                      />
                    </div>
                    <div>
                      <label
                      ...
                      <textarea
                        ...
                        value={pasteContent}
                        onChange={(e) => setPasteContent(e.target.value)}
                        className="block w-full rounded-md border-amber-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 text-sm py-2 px-3 border h-32 bg-white/70"
                      />
                    </div>
                    <div>
                      <label
                ...
                <div className="border border-amber-200 rounded-lg p-6 bg-white/50">
                  <div className="flex items-center mb-4">
                    ...
                    <h4 className="ml-3 text-base font-medium text-amber-900">Article/Blog Post</h4>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <label
                    ...
                    <button
                      onClick={handleSubmitArticle}
                      className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-amber-500 hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 transition-colors"
                    >
                      Publish Article
                    </button>
                  </div>
