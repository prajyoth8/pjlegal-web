            ...
            >
              ...
              {sources.map((s) => (
                <option key={s.id} value={s.type}>
                  {s.name} {s.last_scraped && `(last: ${new Date(s.last_scraped).toLocaleDateString()})`}
                </option>
              ))}
            </select>
            <button
              ...
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-gray-750">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Published</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Source</th>
                ...
                <tbody className="divide-y divide-gray-700">
                  {laws.map((law) => (
                    <tr key={law.id} className="hover:bg-gray-750/50">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-300">
                        {law.published_date ? new Date(law.published_date).toLocaleDateString() : "N/A"}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-blue-300">
                        {law.source || "N/A"}
                      </td>
