...
type KeywordType = "banned" | "legal" | "contact" | "farewell";

export default function KeywordManager() {
  const [keywords, setKeywords] = useState<any[]>([]);
  const [newKeyword, setNewKeyword] = useState("");
  const [newType, setNewType] = useState<KeywordType>("banned");
  ...
  const getTypeStyles = (type: KeywordType) => {
    switch (type) {
      case "banned":
  ...
  const getTypeIcon = (type: KeywordType) => {
    switch (type) {
      case "banned":
        return (
          <svg
  ...
  const getTypeLabel = (type: KeywordType) => {
    switch (type) {
      case "banned":
        return "Banned";
      case "legal":
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <p className="text-blue-200 mt-1">Manage chatbot keywords (banned, legal, contact, and farewell)</p>
            </div>
            <button
              onClick={logout}
