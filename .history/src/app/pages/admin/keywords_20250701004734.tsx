...
export default function KeywordAdmin() {
  ...
  async function fetchKeywords() {
    const { data, error } = await supabase.from("chatbot_keywords").select("*").order("created_at", { ascending: false });
  ...
  async function addKeyword() {
    ...
    const { error } = await supabase.from("chatbot_keywords").insert([
      { keyword: newKeyword.trim(), type: newType },
    ]);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">🛡️ Keyword Manager</h1>
        ...
        <tbody>
          {keywords.map((kw) => (
            <tr key={kw.id} className="border-t">
              ...
              <td className="p-2">{kw.type === "banned" ? "🚫" : "✅"} {kw.type}</td>
              <td className="p-2">
                <button
