...
import { ArrowLeft, Check, Clock, Key, Loader2, Lock, Mail, RefreshCw, Send, ShieldCheck, Smartphone } from "lucide-react";
...
export default function ChatAuthModal({
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      //     method: authMethod,
      //     phone_number: authMethod === "phone" ? contactValue : null,
      //     email: authMethod === "email" ? contactValue : null,
      //   }),
      // });
  ...
  const verifyOtpAndCreateSession = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/auth/verify-otp`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          contact: contactValue, // This is either email or full phone
          otp: otp,
          method: authMethod, // "email" or "phone"
...
//                 />
//               ) : (
//                 <PhoneInputWithCountry value={value} onChange={(val: string) => setValue(val)} />
//               )}
//             </div>
//             <button
//               onClick={sendOtp}
...
return (
    <div className="p-6 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-xl max-w-md w-full mx-auto backdrop-blur-lg">
      <div className="text-center mb-6">
        <div className="flex justify-center mb-3">
          <div className="relative w-20 h-20 rounded-full border-2 border-amber-200 p-1">
            <Image
              src="/assets/ai_avatar.png"
              alt="PJ Legal AI Assistant"
              width={80}
        ...
        {step === "input" ? (
          <>
            <div>
              ...
              <div className="relative">
                ...
                <input
                  id="email-input"
                  type="email"
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg pl-10 pr-4 py-3 text-sm dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all"
                  placeholder="you@example.com"
                  value={value}
                  onChange={(e) => setValue(e.target.value)}
                  autoFocus
            ...
            <button
              ...
              disabled={loading || !value.trim()}
              className={`w-full flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-medium py-3 rounded-lg transition-all ${
                loading || !value.trim() ? "opacity-80 cursor-not-allowed" : "shadow-md hover:shadow-lg"
              }`}
            >
              {loading ? (
                <>
          ...
          <>
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  inputMode="numeric"
                  pattern="[0-9]*"
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg pl-10 pr-4 py-3 text-center text-lg font-mono tracking-widest dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all"
                  placeholder="Enter 6-digit code"
                  value={otp}
                  onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))}
                  autoFocus
              ...
              {!canResend ? (
                <p className="text-sm text-gray-400 mt-2 flex items-center justify-center gap-1">
                  <Clock className="w-3 h-3" /> Resend available in {timer}s
                </p>
              ) : (
                <button
                  onClick={sendOtp}
                  className="text-sm text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 underline mt-2 flex items-center justify-center gap-1 w-full"
                >
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setStep("input")}
                className="flex-1 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 font-medium py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                disabled={loading || otp.length < 6}
                className={`flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-medium py-3 rounded-lg transition-all ${
                  loading || otp.length < 6 ? "opacity-80 cursor-not-allowed" : "shadow-md hover:shadow-lg"
                }`}
              >
                {loading ? (
                  <>
