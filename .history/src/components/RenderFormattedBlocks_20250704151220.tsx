...
rehypePlugins={[rehypeRaw as unknown as Pluggable]}
              components={{
                ...
                ol: ({ children }) => <ol className="list-decimal ml-5 text-sm">{children}</ol>,
                ul: ({ children }) => <ul className="list-disc ml-5 text-sm">{children}</ul>,
              }}
            >
              {block.text}
            </ReactMarkdown>
          ...
          return (
            <button
              key={index}
              onClick={() => (window.location.href = block.action)}
