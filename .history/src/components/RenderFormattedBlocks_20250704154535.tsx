...
type ListGroupBlock = { type: "bulleted_list_group" | "numbered_list_group"; items: { text: string }[] };
      ...
      {blocks.map((block, index) => {
        switch (block.type) {
          ...
          case "heading_4": {
            ...
            return (
              <HeadingTag key={index} className={`text-${level === 1 ? "2xl" : level === 2 ? "xl" : "lg"} font-bold`}>
                {block.text}
              </HeadingTag>
            );
          }
                ...
                <table className="table-auto w-full border border-gray-300">
                  <tbody>
                    {block.rows.map((row, rowIndex) => (
                      <tr
                        key={rowIndex}
                        className={rowIndex === 0 ? "bg-gray-100 font-semibold" : rowIndex % 2 === 0 ? "bg-white" : "bg-gray-50"}
                      >
