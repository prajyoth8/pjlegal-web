...
type HeadingBlock = { type: `heading_${1 | 2 | 3 | 4}`; text: string };
    ...
    <div className="space-y-4">
      {blocks.map((block, index) => {
        switch (block.type) {
          ...
          case "code_block":
            return (
              <pre key={index} className="bg-gray-100 text-sm rounded p-4 overflow-x-auto">
            ...
            return (
              <div key={index} className="overflow-x-auto">
                <table className="w-full border border-gray-300 text-sm">
                  <tbody>
                    {block.rows.map((row, i) => (
                      <tr key={i} className={i === 0 ? "bg-gray-200 font-semibold" : ""}>
                        {row.map((cell, j) => (
                          <td
                            key={j}
          ...
          case "button":
            return (
              <button
                ...
                onClick={() => {
                  if (block.action.startsWith("http")) {
                    window.open(block.action, "_blank");
                  } else {
                    const target = document.querySelector(block.action);
          ...
          default:
            if (block.type.startsWith("heading_")) {
              ...
              const HeadingTag = `h${level}` as keyof JSX.IntrinsicElements;
              return (
                <HeadingTag key={index} className={`text-xl font-bold mt-4`}>
                  <ReactMarkdown rehypePlugins={[rehypeRaw]} remarkPlugins={[remarkGfm]}>
                    {block.text}
                  </ReactMarkdown>
                </HeadingTag>
