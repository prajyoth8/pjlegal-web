...
import { FormattedBlock } from "@/types"; // ✅ FIXED
    ...
    <div className="space-y-4">
      {blocks.map((block, index) => {
        if (block.type === "paragraph" || block.type === "heading") {
          return (
            <ReactMarkdown
              ...
              components={{ p: ({ children }) => <p className="text-base">{children}</p> }}
            >
              {block.text}
            </ReactMarkdown>
          );
        ...
        if (block.type === "bulleted_list_group" || block.type === "numbered_list_group") {
          const ListTag = block.type === "bulleted_list_group" ? "ul" : "ol";
          return (
            <ListTag key={index} className="pl-6 list-disc space-y-1">
