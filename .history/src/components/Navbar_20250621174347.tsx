    ...
    <nav className="fixed top-0 z-50 w-full backdrop-blur-md bg-gradient-to-r from-white/80 to-gray-100/80 shadow-sm transition duration-300">
          ...
          {[
            ...
            <div key={item.id} className="relative">
              <Link
                ...
                className={clsx(
                  "transition",
                  activeSection === item.id ? "text-blue-600 font-semibold border-b-2 border-blue-600" : "text-gray-700 hover:text-blue-600"
                )}
              >
                {item.name}
              </Link>
              {item.dropdown && (
                <div className="absolute left-0 mt-2 w-40 rounded-md shadow-lg bg-white hidden group-hover:block">
                  <Link href="#corporate" className="block px-4 py-2 text-sm hover:bg-gray-100">Corporate</Link>
                  <Link href="#cyberlaw" className="block px-4 py-2 text-sm hover:bg-gray-100">Cyber Law</Link>
                  <Link href="#litigation" className="block px-4 py-2 text-sm hover:bg-gray-100">Litigation</Link>
                </div>
              )}
            </div>
          ))}
      ...
      {menuOpen && (
        <div className="px-6 py-4 space-y-2 md:hidden bg-white shadow-md">
          {["About", "Practice Areas", "Articles/Blogs", "News", "Education", "Contact"].map((item) => (
            <Link
              key={item}
