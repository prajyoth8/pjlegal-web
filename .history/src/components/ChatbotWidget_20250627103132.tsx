...
export default function ChatWidget() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "ai", content: "⚠️ Error getting response. Please try again later." }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const handleInitialGreeting = () => {
    if (messages.length === 0 && isOpen) {
      setMessages([{ 
        role: "ai", 
        content: "Hello! I'm PJ Legal AI Assistant. How can I help you with your legal questions today?" 
      }]);
    }
  };
