  ...
  return (
    <div className="space-y-3">
      {blocks.map((block, index) => {
        switch (block.type) {
          case "paragraph":
            return <p key={index} dangerouslySetInnerHTML={{ __html: block.text }} className="text-base text-gray-700" />;
          case "quote":
            return <blockquote key={index} className="border-l-4 border-gray-400 pl-4 italic text-gray-600">{block.text}</blockquote>;
          case "heading_1":
            return <h1 key={index} className="text-2xl font-bold">{block.text}</h1>;
          case "heading_2":
            return <h2 key={index} className="text-xl font-semibold">{block.text}</h2>;
          case "heading_3":
            return <h3 key={index} className="text-lg font-medium">{block.text}</h3>;
          case "heading_4":
            return <h4 key={index} className="text-md font-medium text-blue-600">{block.text}</h4>;
          case "code_block":
            return <pre key={index} className="bg-gray-100 text-sm rounded p-3 overflow-x-auto"><code>{block.text}</code></pre>;
          case "bulleted_list_group":
            return <ul key={index} className="list-disc pl-5"><RenderListItems items={block.items} /></ul>;
          case "numbered_list_group":
            return <ol key={index} className="list-decimal pl-5"><RenderListItems items={block.items} /></ol>;
          default:
            return null;
        }
      })}
