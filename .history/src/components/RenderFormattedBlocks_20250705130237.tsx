...
const renderListItems = (items: ListItem[], ordered: boolean, level = 0): JSX.Element => {
  const ListTag = ordered ? "ol" : "ul";
  const marginClass = `ml-${Math.min(6, level * 4)}`; // tailwind margin (max ml-6)

  return (
    <ListTag className={`list-${ordered ? "decimal" : "disc"} ${marginClass} pl-4`}>
      {items.map((item, idx) => (
        <li key={idx} className="mb-1">
          <span dangerouslySetInnerHTML={{ __html: item.text }} />
          {item.children &&
            item.children.length > 0 &&
            renderListItems(item.children, ordered, level + 1)}
        </li>
      ))}
    </ListTag>
  );
...
export const RenderFormattedBlocks: React.FC<{ blocks: FormattedBlock[] }> = ({ blocks }) => {
  const renderList = (items: ListItem[], ordered: boolean = false): JSX.Element =>
    ordered ? (
      <ol className="list-decimal pl-5 space-y-1">
        {items.map((item, index) => (
          <li key={index}>
            {parse(item.text)}
            {item.children && renderList(item.children, ordered)}
          </li>
        ))}
      </ol>
    ) : (
      ...
      {blocks.map((block, index) => {
        switch (block.type) {
          ...
          case "heading":
            const HeadingTag = `h${block.level}` as keyof JSX.IntrinsicElements;
            return (
              <HeadingTag key={index} className="font-semibold text-lg">
                {parse(block.text)}
              </HeadingTag>
            );

          case "bulleted_list_group":
            return <div key={index}>{renderListItems(block.items, false)}</div>;
          case "numbered_list_group":
            return <div key={index}>{renderListItems(block.items, true)}</div>;

          case "table":
            return (
              <div key={index} className="overflow-x-auto">
                <table className="table-auto border border-collapse border-gray-400 text-sm text-left w-full">
                  <tbody>
                    {block.rows.map((row, i) => (
                      <tr key={i}>
                        {row.map((cell, j) => (
                          <td
                            key={j}
                            className="border border-gray-300 px-3 py-1 whitespace-pre-wrap"
                          >
                            {parse(cell)}
                          </td>
                        ))}
          ...
          case "button":
            return (
              <button
                key={index}
                className="bg-blue-600 text-white font-semibold px-4 py-2 rounded shadow"
                onClick={() => console.log(`Clicked: ${block.action}`)}
              >
                {block.label}
              </button>
            );
