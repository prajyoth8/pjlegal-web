  ...
  return (
    <div className="space-y-2">
      {blocks.map((block, i) => {
        switch (block.type) {
          case "title":
            return <h1 key={i} className="text-xl font-bold text-blue-900">{block.text}</h1>;

          case "heading":
            return <h2 key={i} className="text-lg font-semibold text-blue-800">{block.text}</h2>;

          case "paragraph":
            return <p key={i} className="text-base text-gray-800">{block.text}</p>;
