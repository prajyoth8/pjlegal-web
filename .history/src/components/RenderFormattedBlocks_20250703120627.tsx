...
export const RenderFormattedBlocks: React.FC<{ blocks: FormattedBlock[] }> = ({ blocks }) => {
  ...
  if ("text" in block) {
    switch (block.type) {
      case "heading_1":
        rendered.push(
          <h1 key={index} className="text-2xl font-bold text-gray-900">{block.text}</h1>
        );
        break;
      case "heading_2":
        rendered.push(
          <h2 key={index} className="text-xl font-bold text-gray-800">{block.text}</h2>
        );
        break;
      case "heading_3":
        rendered.push(
          <h3 key={index} className="text-lg font-semibold text-gray-700">{block.text}</h3>
        );
        break;
      case "heading_4":
      case "subheading":
        rendered.push(
          <h4 key={index} className="text-md font-medium text-blue-600">{block.text}</h4>
        );
        break;
      case "paragraph":
        rendered.push(
