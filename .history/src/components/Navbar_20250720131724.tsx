...
export default function Navbar({
  ...
  const desktopMenuItems = [
    ...
    { name: "Insights", isDropdown: true, href: "#insights" },
  ...
  const insightsSubItems = [
    { name: "News", href: "/insights?type=news" },
    { name: "Articles & Blogs", href: "/insights?type=articles" },
    { name: "Education", href: "/insights?type=education" },
  ];
  ...
  {desktopMenuItems.map((item) => {
    if (item.isDropdown) {
      const isPractice = item.name === "Practice Areas";
      const isInsights = item.name === "Insights";
      const isActive =
        (isPractice && (pathname?.startsWith("/practice-areas") || activeHash === "#practice")) ||
        (isInsights && (pathname?.includes("/insights") || activeHash === "#insights"));

      const subItems = isPractice ? practiceSubItems : insightsSubItems;

      return (
        <div
          key={`dropdown-${item.name}`} // ✅ unique key
          onMouseEnter={() =>
            isInsights ? setSubDropdownOpen("Insights") : setDropdownOpen(true)
          }
          onMouseLeave={() => {
            isInsights ? setSubDropdownOpen(null) : setDropdownOpen(false);
          }}
          className="relative"
        >
          <button
          ...
          <AnimatePresence>
            {(isPractice ? dropdownOpen : subDropdownOpen === "Insights") && (
              <motion.div
    ...
    // Normal links (Home, About, Contact, etc.)
    const href = item.href!;
    return (
      <a
        key={`nav-${item.name}`} // ✅ unique key
