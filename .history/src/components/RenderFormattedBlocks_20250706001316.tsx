...
export const RenderFormattedBlocks: React.FC<{
  ...
  const [viewMode, setViewMode] = useState<"formatted" | "structured" | "dev">("formatted");
  const groupedBlocks = mergeListBlocks(blocks);
  let globalNumbering = 1;

  return (
    <div className="space-y-4 text-[15px] leading-[1.6]">
      {/* Mode Toggle */}
      <div className="flex gap-3 text-sm mb-2 justify-end">
        <button
          onClick={() => setViewMode("formatted")}
          className={`hover:underline ${viewMode === "formatted" ? "font-bold text-blue-600" : ""}`}
        >
          📝 Text View
        </button>
        <button
          onClick={() => setViewMode("structured")}
          className={`hover:underline ${viewMode === "structured" ? "font-bold text-green-600" : ""}`}
        >
          🧱 Block View
        </button>
        <button
          onClick={() => setViewMode("dev")}
          className={`hover:underline ${viewMode === "dev" ? "font-bold text-red-600" : ""}`}
        >
          🔧 Dev Mode
        </button>
      </div>

      {/* View Mode: Markdown/Formatted */}
      {viewMode === "formatted" && raw_text && (
        <ReactMarkdown
      ...
      {viewMode === "structured" &&
        groupedBlocks.map((block, index) => {
          if (block.type === "heading" || block.type === "title" || block.type.startsWith("heading_")) {
            globalNumbering = 1;
          }

          if (block.type === "paragraph") return <p key={index}>{parse(block.text)}</p>;
