...
export type FormattedBlock = ParagraphBlock | ListGroupBlock | ButtonBlock;

export const RenderFormattedBlocks: React.FC<{ blocks: FormattedBlock[] }> = ({ blocks }) => {
  const renderListItems = (items: ListItem[], type: "numbered_list_group" | "bulleted_list_group") => {
    const Tag = type === "numbered_list_group" ? "ol" : "ul";

    return (
      <Tag className={type === "numbered_list_group" ? "list-decimal pl-5" : "list-disc pl-5"}>
        {items.map((item, idx) => {
          if (item.text) {
            return (
              <li key={idx} dangerouslySetInnerHTML={{ __html: item.text }} className="text-gray-700" />
            );
          } else if (item.items && item.type) {
            // Nested list
            return (
              <li key={idx}>
                {renderListItems(item.items, item.type)}
              </li>
            );
          }
          return null;
