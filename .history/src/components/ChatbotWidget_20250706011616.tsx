...
import { RenderFormattedBlocks } from "./RenderFormattedBlocks";
...
const ContactButtons = ({ router }: { router: ReturnType<typeof useRouter> }) => (
...
export default function ChatWidget() {
  ...
  type ChatMessage =
    ...
    | { role: "ai"; content: string | FormattedBlock[] | JSX.Element };
  ...
  const onAuthenticated = (sessionId: string, emailOrPhone: string) => {
    ...
    setMessages((prev) => [
      ...prev,
      { role: "ai", content: `✅ Authenticated successfully. Welcome! ${emailOrPhone}.` },
                ...
                >
                  {Array.isArray(msg.content) ? (
                    <RenderFormattedBlocks
                      blocks={msg.content as FormattedBlock[]}
                      rawText={(msg.content as FormattedBlock[])
                        .filter((b) => "text" in b && typeof b.text === "string")
                        .map((b) => (b as any).text)
                        .join("\n")}
                    />
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (typeof msg.content === "string") {
                          ...
                          // JSX.Element case: optional fallback
                          speakText = "Here is the response.";
                        }
