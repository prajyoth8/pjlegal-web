...
export const RenderFormattedBlocks: React.FC<{ blocks: FormattedBlock[] }> = ({ blocks }) => {
  ...
  const renderListGroup = (
    list: ParagraphBlock[],
    level = 0,
    parentType: "bulleted_list" | "numbered_list"
  ): JSX.Element => {
    const Tag = parentType === "bulleted_list" ? "ul" : "ol";

    const items = list.map((item, index) => {
      const childList: ParagraphBlock[] = [];
      const rest: ParagraphBlock[] = [];

      // Group items with deeper nesting under this one
      for (let i = index + 1; i < list.length; i++) {
        const next = list[i];
        if ((next.level ?? 0) > (item.level ?? 0)) {
          childList.push(next);
        } else {
          rest.push(next);
          break;
      ...
      return (
        <li key={index} className="text-gray-700" dangerouslySetInnerHTML={{ __html: item.text }}>
          {childList.length > 0 &&
            renderListGroup(childList, childList[0].level ?? 0, childList[0].type)}
        </li>
      );
    });

    return (
      <Tag className="pl-5" key={Math.random()}>
        {items}
      </Tag>
    );
  };
  ...
  const flushList = () => {
    ...
    rendered.push(renderListGroup(tempList, tempList[0].level ?? 0, tempList[0].type));
    tempList = [];
  };
