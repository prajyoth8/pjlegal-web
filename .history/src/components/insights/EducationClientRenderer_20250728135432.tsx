...
export default function EducationClientRenderer() {
  ...
  return (
    <div className="space-y-6">
      {educationTopics.map((section: Section) => (
        <div
          key={section.id}
        ...
        >
          ...
          {openSection === section.title && (
            <div className="px-5 py-4 bg-white dark:bg-gray-900 rounded-b-md">
              {section.sections.map((item: SubSection, index: number) => (
                <div key={index} className="mb-4">
                  ...
                  {item.truth !== undefined && (
                    <p className="mt-2 text-gray-800 dark:text-gray-300">
                      <strong className="text-red-600">Myth:</strong> {item.title}
                      <br />
