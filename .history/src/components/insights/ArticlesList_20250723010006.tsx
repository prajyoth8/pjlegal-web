...
//       // Supabase returns created_by as an array even for single relations
//       setArticles(data || []);
//     } catch (err) {
...
//           <div
//             key={i}
//             className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow"
//           >
...
//                   <Image
//                     src={article.image_url || "/placeholder.jpg"} // Fallback image
//                     alt={article.title}
//                     fill
...
import Link from 'next/link'
import Image from 'next/image'
import { formatDate } from '@/lib/utils' // Create this helper
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      {articles.map((article) => (
        <article key={article.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
          {/* Image thumbnail with proper aspect ratio */}
          {article.image_url && (
            <div className="relative h-48 w-full">
              <Image
          ...
          <div className="p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {article.tags?.map((tag) => (
                <span key={tag} className="text-xs px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full">
                  {tag}
                </span>
              ))}
            </div>
