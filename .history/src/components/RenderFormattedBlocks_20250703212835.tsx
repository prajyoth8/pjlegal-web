...
export const RenderFormattedBlocks: React.FC<{ blocks: FormattedBlock[] }> = ({ blocks }) => {
  const renderList = (
    ...
    const ListTag = type === "bulleted_list" ? "ul" : "ol";

    return (
      <ListTag className={`pl-5 ${type === "bulleted_list" ? "list-disc" : "list-decimal"}`}>
        {items.map((item, idx) => (
  ...
  const renderBlocks = (blocks: FormattedBlock[]): JSX.Element[] => {
    const rendered: JSX.Element[] = [];
    ...
    blocks.forEach((block, index) => {
      ...
      switch (b.type) {
        case "heading_1":
          rendered.push(
            <h1 key={index} className="text-2xl font-bold text-gray-900" dangerouslySetInnerHTML={{ __html: b.text }} />
          );
          break;
        case "heading_2":
          rendered.push(
            <h2 key={index} className="text-xl font-bold text-gray-800" dangerouslySetInnerHTML={{ __html: b.text }} />
          );
          break;
        case "heading_3":
          rendered.push(
            <h3 key={index} className="text-lg font-semibold text-gray-700" dangerouslySetInnerHTML={{ __html: b.text }} />
          );
          break;
        case "heading_4":
        case "subheading":
          rendered.push(
            <h4 key={index} className="text-md font-medium text-blue-600" dangerouslySetInnerHTML={{ __html: b.text }} />
          );
          break;
        case "paragraph":
          rendered.push(
            <p key={index} className="text-base text-gray-700" dangerouslySetInnerHTML={{ __html: b.text }} />
          );
          break;
        case "quote":
          rendered.push(
        ...
        case "bulleted_list":
        case "numbered_list":
          // collect consecutive list blocks of same type
          const group: ParagraphBlock[] = [b];
          let i = index + 1;
          while (i < blocks.length && blocks[i].type === b.type) {
            group.push(blocks[i] as ParagraphBlock);
            i++;
          }
          rendered.push(renderList(group, b.type));
          blocks.splice(index + 1, group.length - 1); // skip next N already grouped
          break;
      }
    });
