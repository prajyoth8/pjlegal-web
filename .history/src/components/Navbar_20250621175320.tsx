...
export default function Navbar() {
  ...
  const searchIndex = [
    ...
    { label: "Litigation", route: "/practice-areas/litigation" },
    { label: "Contracts", route: "/practice-areas/contracts" },
    { label: "IP & Trademarks", route: "/practice-areas/ip" },
    { label: "Legal Advisory", route: "/practice-areas/advisory" },
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      if (searchText.trim()) {
        const result = fuse.search(searchText.trim());
        const formatted = result.map(({ item, matches }) => {
          return {
            ...item,
            matchIndices: matches?.[0]?.indices || [],
          };
        });
        ...
        {showSearch && (
          ...
          >
            ...
            {suggestions.length > 0 && (
              <div className="bg-white shadow rounded-lg border border-gray-200 mt-2 max-h-60 overflow-y-auto">
                {suggestions.map((s) => (
                  <button
                    key={s.route}
                    onClick={() => {
                    ...
                    className="w-full text-left px-4 py-2 hover:bg-blue-50 text-sm text-gray-800"
                  >
                    {highlightMatch(s.label, s.matchIndices)}
                  </button>
                ))}
              </div>
            )}
