    ...
    <div className="space-y-3">
      {blocks.map((block, i) => {
        switch (block.type) {
          case "heading":
            return (
              <h2 key={i} className="text-lg font-bold text-amber-700">
                {block.text}
              </h2>
            );
          case "subheading":
            return (
              <h3 key={i} className="text-base font-semibold text-amber-600">
                {block.text}
              </h3>
            );
          case "paragraph":
          ...
          case "image":
            return (
              <div key={i} className="my-3">
                ...
                {block.caption && (
                  <p className="text-xs text-center text-gray-500 mt-1">{block.caption}</p>
                )}
              </div>
            );
          case "button":
