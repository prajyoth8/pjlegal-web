      ...
      {blocks.map((block, index) => {
        ...
        if (block.type === "bulleted_list_group" || block.type === "numbered_list_group") {
          ...
          return (
            <ListTag
              key={index}
              className="pl-5 list-inside"
              style={{ listStyleType: block.type === "numbered_list_group" ? "decimal" : "disc" }}
            >
                  ...
                  {item.children && (
                    <ul className="ml-6 list-disc text-sm">
                      {item.children.map((child, childIdx) => (
                        <li key={childIdx}>{parse(child.text)}</li>
                      ))}
                    </ul>
                  )}
                </li>
        ...
        if (block.type === "table") {
          return (
            <table
              key={index}
              className="table-auto border-collapse border border-gray-400 text-sm"
            >
              <tbody>
                {block.rows.map((row, rowIdx) => (
                  <tr key={rowIdx} className={rowIdx === 0 ? "font-bold bg-gray-100" : ""}>
                    {row.map((cell, cellIdx) => (
                      <td key={cellIdx} className="border border-gray-300 px-3 py-1">
                        {parse(cell)}
                      </td>
                    ))}
                  </tr>
        ...
        if (block.type === "code_block") {
          return (
            <pre key={index} className="bg-gray-900 text-white text-sm p-3 rounded overflow-x-auto">
        ...
        if (block.type === "button") {
          return (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block bg-blue-600 text-white font-semibold px-4 py-2 rounded shadow hover:bg-blue-700 transition"
            >
              {block.label}
            </a>
          );
