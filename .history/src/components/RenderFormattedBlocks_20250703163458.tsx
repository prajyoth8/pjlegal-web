...
const RenderListItem: React.FC<{ item: ListItem | ListBlock, depth?: number }> = ({ item, depth = 0 }) => {
  if ('list_type' in item) {
    ...
    return (
      <li className={`text-gray-700 ${depth > 0 ? 'pl-4' : ''}`}>
        ...
        {item.items && (
          <RenderList items={item.items} type={item.list_type === 'ordered' ? 'ordered_list' : 'unordered_list'} depth={depth + 1} />
        )}
      </li>
    );
  }
...
const RenderList: React.FC<{ items: (ListItem | ListBlock)[], type: 'ordered_list' | 'unordered_list', depth?: number }> = 
  ({ items, type, depth = 0 }) => {
    const ListTag = type === 'ordered_list' ? 'ol' : 'ul';
    const listClass = type === 'ordered_list' 
      ? 'list-decimal pl-5' 
      : 'list-disc pl-5';
