...
export default function HybridLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Top Navbar (shifts on desktop when sidebar is open) */}
      <div className={clsx("relative z-50", sidebarOpen ? "lg:ml-64" : "")}>
        <Navbar toggleSidebar={toggleSidebar} sidebarOpen={sidebarOpen} />
      </div>
      ...
      <div className="flex flex-1 relative">
        ...
        >
          {/* Mobile Top Controls (if needed) */}
          <div className="lg:hidden flex justify-between items-center px-4 pt-4">
            <button
              className="p-2 bg-white border border-gray-300 text-black rounded-md shadow-md"
          ...
          {/* Main Page Content */}
          <div className="pt-16 min-h-[calc(100vh-4rem)] px-4">{children}</div>
