...
export default function CarouselSection() {
  ...
  return (
    <section id="carousel" className="py-16 bg-gray-900 relative snap-start min-h-screen">
      ...
      {modalData && (
        <div
          id="modal-backdrop"
          className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center px-4 py-10 transition-opacity"
          onClick={handleBackdropClick}
        >
          <div className="bg-gray-900 rounded-xl max-w-5xl w-full p-6 relative flex flex-col lg:flex-row gap-6 max-h-[90vh] overflow-y-auto">
            <button
              className="sticky top-2 left-full z-50 text-white hover:text-red-500 bg-black/60 backdrop-blur-md rounded-full p-1.5"
      ...
      {/* Scroll to next section from carousel */}
      <div
        className="mt-12 cursor-pointer group text-center"
        onClick={() => {
          const carousel = document.getElementById("carousel");
          if (carousel) {
            const next = carousel.nextElementSibling as HTMLElement;
            next?.scrollIntoView({ behavior: "smooth" });
          }
        }}
      >
        <svg
          className="w-8 h-8 text-white group-hover:text-yellow-400 animate-bounce drop-shadow-md transition duration-300 mx-auto"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          viewBox="0 0 24 24"
        ...
        <p className="text-sm text-gray-300 mt-1 group-hover:text-yellow-400 transition">
          Scroll Down
        </p>
      </div>
    </section>
